...
resourcename="shoppingcartapp"  # lower case, only
resourcegroup="shoppingcartapp_rg"
...
# git clone https://github.com/apilogicserver/shoppingcartapp.git
# cd shoppingcartapp
...
az group create --name shoppingcartapp_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group shoppingcartapp_rg --sku S1 --is-linux
az container create --resource-group shoppingcartapp_rg --name shoppingcartapp --image apilogicserver/shoppingcartapp:latest --dns-name-label shoppingcartapp --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//shoppingcartapp.westus.azurecontainer.io:5656'
