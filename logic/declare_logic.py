...
def declare_logic():
    ...
    # Customer balance derived as sum of orders where date_shipped is null.
    Rule.sum(derive=Customer.balance, as_sum_of=Order.amount_total)

    # Order amount total is derived as the sum of item amounts.
    Rule.sum(derive=Order.amount_total, as_sum_of=Item.amount)

    # Item amount derived as quantity times unit price.
    Rule.formula(derive=Item.amount, as_expression=lambda row: row.quantity * row.unit_price)

    # Copy item unit price from product unit price.
